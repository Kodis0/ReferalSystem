...
- **Реферальный код**: `ref`, `partner_ref`, `referral`, `ReferralCode` — должен совпасть с тем, что в ссылке `/?ref=...`. Если отдельного поля нет, бэкенд пытается взять `ref` из любого значения, похожего на URL (часто приходит `pageurl` / `PageUrl` у вебхука магазина Tilda). Для **виджета** на странице: если в JSON нет `ref`, но в `page_url` есть полная ссылка с `?ref=`, партнёр подставляется из неё.
